...
void *ThreadMerge(void *threadarg);

// new struct for holding merge function arguments
struct merge_args{
	int first;
	int midpt;
	int last;
...
// array to hold the thread argument structure
struct merge_args merge_args_array[(N/2)-1];

// array to hold threads
pthread_t threads[(N/2)-1];

// variable for joining threads
void *status;
...
while (arrsize < N) {
   ...
   for (i=0; i<N; i+=arrsize) {
      ...
      merge_args_array[j].first = first;
      merge_args_array[j].midpt = midpt;
      merge_args_array[j].last = last;
      pthread_create(&threads[j], NULL, ThreadMerge, (void *) &merge_args_array[j]);
      j++; 
   }
...
struct merge_args *indicies;
indicies = (struct merge_args *) threadarg;
merge(indicies->first, indicies->midpt, indicies->last);
pthread_exit(NULL);
}
